...
class ColabTrainer:
    ...
    def load_sample_data(self) -> Tuple[jnp.ndarray, Dict[str, jnp.ndarray]]:
        ...
        perceptual_dims = [
            ...
            "Articulation_Soft_cushioned_Hard_solid", "Pedal_Sparse/dry_Saturated/wet",
            "Pedal_Clean_Blurred", "Timbre_Even_Colorful", "Timbre_Shallow_Rich",
            "Timbre_Bright_Dark", "Timbre_Soft_Loud", "Dynamic_Sophisticated/mellow_Raw/crude",
            "Dynamic_Little_dynamic_range_Large_dynamic_range", "Music_Making_Fast_paced_Slow_paced",
            "Music_Making_Flat_Spacious", "Music_Making_Disproportioned_Balanced",
            "Music_Making_Pure_Dramatic/expressive", "Emotion_&_Mood_Optimistic/pleasant_Dark",
            "Emotion_&_Mood_Low_Energy_High_Energy", "Emotion_&_Mood_Honest_Imaginative",
            "Interpretation_Unsatisfactory/doubtful_Convincing"
        ]
        
        labels = {
            dim: jnp.array(raw_labels[:, i]) 
    ...
    def create_data_batches(self, spectrograms: jnp.ndarray, labels: Dict, batch_size: int = 8):
    ...
    def train_ast_finetuning(self, spectrograms: jnp.ndarray, labels: Dict, 
                           ssast_state = None, num_epochs: int = 10):
    ...
    def run_complete_training(self):
        ...
        ssast_state, pretraining_losses = self.train_ssast_pretraining(
            spectrograms, 
            num_epochs=self.config.get('pretraining_epochs', 3)
        )
